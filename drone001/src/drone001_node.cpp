 ...
 * 包含无人机名称和位置坐标信息
 */
class Drone001Node : public rclcpp::Node
{
public:
  /**
  ...
  : Node("drone001_node")
  {
    ...
    timer_ = this->create_wall_timer(
      1s, std::bind(&Drone001Node::timer_callback, this));
    
    // 初始化无人机状态数据
    drone_state_.drone_name = "drone001";
    drone_state_.x = 0.0;
    drone_state_.y = 0.0;
    drone_state_.z = 0.0;
    
    RCLCPP_INFO(this->get_logger(), "drone001_node 节点已启动");
  }
 ...
 * @brief 主函数
 * 
 * 初始化 ROS2 客户端库，创建 drone001_node 节点并 spin
 */
int main(int argc, char * argv[])
{
  ...
  // 创建 drone001_node 节点实例
  auto node = std::make_shared<Drone001Node>();
